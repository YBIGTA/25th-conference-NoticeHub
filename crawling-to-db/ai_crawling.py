...
def scrape_page_ai(offset):
    """인공지능융합대학 공지사항에서 특정 페이지의 데이터를 크롤링"""
    ...
    for row in soup.select('tr'):
        title_tag = row.select_one('div.bo_tit a')
...
def scrape_notice_context(link, title):
    ...
    # 이미지 업로드
    image_tags = soup.select('div#bo_v_con img')
    image_urls = []
...
def crawl_ai():
    """인공지능융합대학의 모든 공지사항 데이터를 반환"""
    all_notices = []
    for page in range(1, 6):
        print(f"인공지능융합대학 {page}페이지 크롤링 중...")
        all_notices.extend(scrape_page_ai(page))
    return all_notices