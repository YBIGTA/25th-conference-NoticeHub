...
def scrape_page_german(offset):
    """독어독문학과 공지사항 특정 페이지 크롤링"""
    ...
    for row in soup.select('tr'):
        ...
        if title_tag and tds:
            ...
            if re.match(r'^\d{2}\.\d{2}\.\d{2}$', raw_date):
                date = f"20{raw_date}"  # "YYYY.MM.DD" 형식으로 변환
            else:
                continue
...
def crawl_german_language_literature():
    """독어독문학과의 모든 공지사항 데이터를 반환"""
    all_notices = []
    for page in range(5):  # 1페이지부터 5페이지까지 크롤링
        offset = page * 10
        print(f"독어독문학과 {page + 1}페이지 크롤링 중...")
        all_notices.extend(scrape_page_german(offset))
    return all_notices
