...
def scrape_page_korean(offset):
    """국어국문학과 공지사항 특정 페이지 크롤링"""
    ...
    for row in soup.select('tr'):
        ...
        if title_tag and td_tags and len(td_tags) > 1:
            ...
            raw_date = td_tags[-1].get_text(strip=True)
            date = f"20{raw_date}" if re.match(r'^\d{2}\.\d{2}\.\d{2}$', raw_date) else "날짜 없음"
...
def crawl_korean():
    """국어국문학과의 모든 공지사항 데이터를 반환"""
    all_notices = []
    for page in range(5):
        offset = page * 10
        print(f"국어국문학과 {page + 1}페이지 크롤링 중...")
        all_notices.extend(scrape_page_korean(offset))
    return all_notices
