...
def scrape_page_music(offset):
    """음악대학 공지사항에서 특정 페이지의 데이터를 크롤링"""
...
def scrape_notice_context(link, title):
    """음악대학 공지사항 상세 페이지에서 내용을 크롤링하고 이미지를 S3에 업로드"""
...
def crawl_music():
    """음악대학의 모든 공지사항 데이터를 반환"""
    all_notices = []
    for page in range(5):
        offset = page * 10
        print(f"음악대학 {page + 1}페이지 크롤링 중...")
        all_notices.extend(scrape_page_music(offset))
    return all_notices