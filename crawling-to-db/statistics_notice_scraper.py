...
def scrape_page_statistics(offset):
    """응용통계학과 공지사항 특정 페이지 크롤링"""
...
def crawl_statistics():
    """응용통계학과의 모든 공지사항 데이터를 반환"""
    all_notices = []
    for page in range(5):  # 1페이지부터 5페이지까지 크롤링
        offset = page * 10
        print(f"응용통계학과 {page + 1}페이지 크롤링 중...")
        all_notices.extend(scrape_page_statistics(offset))
    return all_notices