...
def scrape_page_russian(offset):
    """노어노문학과 공지사항 특정 페이지 크롤링"""
...
def crawl_russian_language_literature():
    """노어노문학과의 모든 공지사항 데이터를 반환"""
    all_notices = []
    for page in range(5):  # 1페이지부터 5페이지까지 크롤링
        offset = page * 10
        print(f"노어노문학과 {page + 1}페이지 크롤링 중...")
        all_notices.extend(scrape_page_russian(offset))
    return all_notices
