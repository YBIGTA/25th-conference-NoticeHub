...
def scrape_page_social_sciences(offset):
    """사회과학대학 공지사항에서 특정 페이지의 데이터를 크롤링"""
...
def crawl_social_sciences():
    """사회과학대학의 모든 공지사항 데이터를 반환"""
    all_notices = []
    for page in range(5):
        offset = page * 10
        print(f"사회과학대학 {page + 1}페이지 크롤링 중...")
        all_notices.extend(scrape_page_social_sciences(offset))
    return all_notices